...
This document is written rather formally for sake of precision, and readers may want to read [Concise Rules](/markdown/concise-rules.md) first, if they have not done so.
...
Each move takes a _tempo_, a discrete time index.
...
1. When a Lord goes to an empty square, a lifeless body taken from the earth may be _summoned_ on the square the Lord formerly occupied.
The summoned body must be of the same color as the Lord, and other friendly men may not do so.
The action takes a tempo.
...
When a Soldier is already in the furthest rank from the start position (i.e., any square from _a8_ to _h8_ for White, and that from _a1_ to _h1_ for Black), and either or both Priestess body is absent from the board, it may be _promoted_ to a lifeless Priestess.
The body and soul of the Soldier shall stay in the earth and the underworld, respectively, as usual.

   - According to the definition here, it takes a tempo to move from the second-to-last rank to the last rank, and it takes another tempo (if it is not captured in that round).
...
1. A friendly man _attacks_ a living body, ill-suited body, or lifeless body, the man is about to capture the latter in the next round, if it were to make another move on the present position.
...
1. And the game results in a draw if there are 32 consecutive _quiet_ rounds, which means that: no man has ever captured, no soldier has ever moved, no Priestess has ever fetched a soul, and no Lord has ever summoned a body.
